...
export function buildOpenAIVisionPayload(model:string,image:ArrayBuffer):unknown{
  ...
  const prompt=[
    ...
    "For receipts, tax invoices and online orders, copy only visible facts. Extract every visible line item up to 30 rows. Do not truncate a long receipt. finalPaidAmountBaht is the final cash outflow after shipping, discounts, vouchers and subsidies. Do not use expected delivery as paymentDate. DELIVERY_ORDER is supporting evidence and needsReview=true unless visible payment evidence is present.",
  ...
  return{model,store:false,max_output_tokens:2600,text:{format:{type:"json_schema",name:"malipang_image_read",strict:true,schema}},input:[{role:"user",content:[{type:"input_text",text:prompt},{type:"input_image",image_url:`data:image/jpeg;base64,${arrayBufferToBase64(image)}`,detail:"high"}]}]};
}
...
function normalizePurchaseDocument(value:unknown):PurchaseDocument|null{
  ...
  const category=text(obj.suggestedCategory)||"general",items=Array.isArray(obj.items)?obj.items.map(item=>{const row=item&&typeof item==="object"?item as Record<string,unknown>:{};return{sellerKey:text(row.sellerKey),productCode:text(row.productCode),description:text(row.description),quantity:money(row.quantity),unit:text(row.unit),unitPriceBaht:money(row.unitPriceBaht),discountBaht:money(row.discountBaht),lineTotalBaht:money(row.lineTotalBaht),vatBaht:money(row.vatBaht),confidence:Number(row.confidence||0),needsReview:Boolean(row.needsReview)} satisfies ExpenseDocumentItem;}).filter(item=>item.description):[];
