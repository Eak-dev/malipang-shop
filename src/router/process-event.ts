...
export async function processInbound(job:InboundJob,env:Env,_ctx:ExecutionContext):Promise<void>{
  ...
  try{
    ...
    if(event.message.contentProvider?.type&&event.message.contentProvider.type!=="line"){
      if(actor)await enqueueAttendanceNotification(env,{to,text:unsupportedImageMessage(),identity:`attendance:${webhookId}:rejection`,purpose:"ATTENDANCE_REJECTION",traceId:job.traceId});
    ...
    if(!actor||(!(actor.status==="ACTIVE"&&env.ATTENDANCE_ENABLED==="true")&&!(actor.canSubmitExpense&&env.EXPENSE_ENABLED==="true"))){
      if(actor)await enqueueAttendanceNotification(env,{to,text:unauthorizedImageMessage(),identity:`attendance:${webhookId}:rejection`,purpose:"ATTENDANCE_REJECTION",traceId:job.traceId});
    ...
    }else{const rejection=describeVisionRejection(reading);await enqueueAttendanceNotification(env,{to,text:rejection.message,identity:`attendance:${webhookId}:rejection`,purpose:"ATTENDANCE_REJECTION",traceId:job.traceId});await completeInboundEvent(env,webhookId,"UNKNOWN_IMAGE","REVIEW",rejection.code);}
