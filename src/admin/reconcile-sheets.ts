...
export async function reconcileSheets(env:Env,input:ReconcileInput={}):Promise<{fromDate:string;toDate:string;enqueued:number;counts:Record<string,number>}>{
  ...
  const [attendance,daily,weekly,expense]=await Promise.all([
  ...
  const grouped={ATTENDANCE_EVENT:jobs(attendance.results||[],"ATTENDANCE_EVENT",traceId),DAILY_PAYROLL:jobs(daily.results||[],"DAILY_PAYROLL",traceId),WEEKLY_PAYROLL:jobs(weekly.results||[],"WEEKLY_PAYROLL",traceId),EXPENSE:jobs(expense.results||[],"EXPENSE",traceId)};
