...
function replyFlex(env:Env,event:LineEvent,message:unknown,traceId:string,suffix="hr-flex"):Promise<unknown>{return respondFlexToLineEvent(env,event,message,{traceId,purpose:"EMPLOYEE_RESPONSE",identitySuffix:suffix});}
...
function requestTime(value:string):string{try{return new Intl.DateTimeFormat("th-TH",{timeZone:"Asia/Bangkok",day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:false}).format(new Date(value));}catch{return value;}}
...
function onboardingBubble(request:HrOnboardingRequest):unknown{return{
  type:"bubble",
  ...
  footer:{type:"box",layout:"vertical",spacing:"sm",contents:[
    button("✅ เพิ่มเป็นพนักงานใหม่",`HR ADD ${request.requestId}`,"primary"),
    button("🔗 ใช้ข้อมูลพนักงานเดิม",`HR STAFF ${request.requestId}`),
...
function addConfirmFlex(request:HrOnboardingRequest):unknown{return{type:"flex",altText:`ยืนยันเพิ่มพนักงาน ${safeText(request.displayName)}`,contents:{type:"bubble",...hero(request.pictureUrl),body:{type:"box",layout:"vertical",spacing:"sm",contents:[
...
function staffChoiceBubble(request:HrOnboardingRequest,staff:UnboundStaff):unknown{return{type:"bubble",body:{type:"box",layout:"vertical",spacing:"sm",contents:[
  textBlock("🔗 เชื่อมกับพนักงานเดิม","lg","bold"),
...
function linkConfirmFlex(request:HrOnboardingRequest,staff:UnboundStaff):unknown{return{type:"flex",altText:`ยืนยันเชื่อม ${staff.employeeId}`,contents:{type:"bubble",...hero(request.pictureUrl),body:{type:"box",layout:"vertical",spacing:"sm",contents:[
...
async function ownerOnboardingCommand(env:Env,event:LineEvent,actor:StaffActor|null,traceId:string,text:string,deps:HrDependencies):Promise<boolean>{
  ...
  const action=match[1]!.toUpperCase(),args=match[2]!.trim().split(/\s+/).filter(Boolean),getRequest=deps.getOnboarding||getLineFirstOnboardingRequest,listStaff=deps.listUnboundStaff||listUnboundActiveStaff;
  try{
    if(action==="ADD"){
      const confirm=args[0]?.toUpperCase()==="CONFIRM",requestId=confirm?args[1]:args[0];
    ...
    if(action==="STAFF"){
      ...
      if(!staff.length){await reply(env,event,`ไม่พบพนักงาน ACTIVE ที่ยังไม่ได้เชื่อม LINE\nหากเป็นพนักงานใหม่ ให้พิมพ์ HR ADD ${requestId}`,traceId);return true;}
      await replyFlex(env,event,staffChoicesFlex(request,staff),traceId,"hr-staff-list");return true;
    }
    if(action==="LINK"){
      const confirm=args[0]?.toUpperCase()==="CONFIRM",requestId=confirm?args[1]:args[0],employeeId=confirm?args[2]:args[1];
      ...
      if(!confirm){if(request.status!=="PENDING_OWNER_SETUP"){await reply(env,event,`คำขอนี้อยู่ในสถานะ ${request.status}`,traceId);return true;}const staff=(await listStaff(env)).find(item=>item.employeeId===employeeId);if(!staff){await reply(env,event,"ไม่พบพนักงานนี้ในรายการที่ยังเชื่อม LINE ได้",traceId);return true;}await replyFlex(env,event,linkConfirmFlex(request,staff),traceId,"hr-link-confirm");return true;}
...
export async function handleHrText(env:Env,event:LineEvent,actor:StaffActor|null,traceId:string,deps:HrDependencies={}):Promise<boolean>{
  ...
  // Compatibility path for staff identities provisioned before LINE-first onboarding.
  ...
  if(!actor&&/^[A-Za-z0-9_-]{1,40}$/.test(text)){
    ...
    // Allow an already-started legacy registration to finish during transition.
