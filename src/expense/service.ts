...
export async function handleExpensePostback(env:Env,event:LineEvent,actor:Employee):Promise<void>{
  ...
  if(action==="expense_set_payment"){
    ...
    await env.DB.prepare(`UPDATE expense_events SET payment_key=?,source_wallet=?,updated_at=? WHERE expense_id=? AND line_user_id=? AND status='WAITING_CONFIRM'`).bind(expense.paymentKey,expense.sourceWallet,new Date().toISOString(),id,to).run();await respondFlex(env,event,buildExpenseSummaryFlex(expense),traceId);return;
  }
  ...
  if(action==="expense_set_category"){
    ...
    await env.DB.prepare(`UPDATE expense_events SET category=?,updated_at=? WHERE expense_id=? AND line_user_id=? AND status='WAITING_CONFIRM'`).bind(category,new Date().toISOString(),id,to).run();await respondFlex(env,event,buildExpenseSummaryFlex(expense),traceId);return;
  }
  if(action==="expense_set_date_rel"){
    ...
    await env.DB.prepare(`UPDATE expense_events SET transaction_date=?,updated_at=? WHERE expense_id=? AND line_user_id=? AND status='WAITING_CONFIRM'`).bind(expense.transactionDate,new Date().toISOString(),id,to).run();await respondFlex(env,event,buildExpenseSummaryFlex(expense),traceId);return;
  }
  if(action==="expense_set_date"){
    ...
    await env.DB.prepare(`UPDATE expense_events SET transaction_date=?,updated_at=? WHERE expense_id=? AND line_user_id=? AND status='WAITING_CONFIRM'`).bind(date,new Date().toISOString(),id,to).run();await respondFlex(env,event,buildExpenseSummaryFlex(expense),traceId);return;
  }
