...
export async function importEmployees(env:Env,inputs:EmployeeImportInput[]):Promise<void>{
  ...
  for(const e of inputs){if(!/^[A-Za-z0-9_-]{1,40}$/.test(e.employeeId)||!e.staffName.trim()||!/^U[a-fA-F0-9]{20,64}$/.test(e.lineUserId)||!/^\d{2}:\d{2}$/.test(e.scheduledIn)||!/^\d{2}:\d{2}$/.test(e.scheduledOut)||!Number.isFinite(e.dailyWageBaht)||e.dailyWageBaht<0||!Number.isFinite(e.graceMin)||e.graceMin<0||!Number.isFinite(e.lateDeductionBaht)||e.lateDeductionBaht<0||(e.earlyDeductionBaht!==undefined&&(!Number.isFinite(e.earlyDeductionBaht)||e.earlyDeductionBaht<0)))throw new Error(`Invalid employee: ${e.employeeId||"unknown"}`);if(ids.has(e.employeeId)||lines.has(e.lineUserId))throw new Error(`Duplicate employee or LINE ID: ${e.employeeId}`);ids.add(e.employeeId);lines.add(e.lineUserId);}
