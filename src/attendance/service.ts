...
export async function handleAttendance(env:Env,event:LineEvent,employee:Employee,reading:VisionResult,original:ArrayBuffer,traceId:string):Promise<boolean>{
  ...
  if(!validated.ok){
    await enqueueAttendanceNotification(env,{to,text:describeClockValidationFailure(validated.validationCode).message,identity:`attendance:${webhookEventId}:rejection`,purpose:"ATTENDANCE_REJECTION",traceId});
    return false;
  }
  ...
  const notification={to,text:buildAttendanceReply(employee,result),identity:`attendance:${webhookEventId}:result`,purpose:"ATTENDANCE_RESULT" as const,traceId};
