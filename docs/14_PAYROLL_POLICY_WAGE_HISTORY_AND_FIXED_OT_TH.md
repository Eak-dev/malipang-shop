...
5. ห้ามตีความ `Effective_From` รายการแรกเป็นวันเริ่มจ้างโดยอัตโนมัติ เพราะอาจเป็น migration baseline หากต้องตัดค่าแรงก่อนเริ่มงานต้องมีหลักฐานวันเริ่มงานจริงและปรับเฉพาะพนักงาน/วันที่ที่ยืนยันแล้ว พร้อม Audit
...
ระบบคำนวณจากเวลาสายจริงเทียบกับ `Scheduled_In` เฉพาะเมื่อพนักงานตั้ง `Deduct_Late = TRUE` เท่านั้น ถ้าเป็น `FALSE` ต้องไม่หักมาสายทุกกรณี และช่วงผ่อนผันใช้ค่า `Grace_Min` ของพนักงาน ไม่ใช้ค่าตายตัว

ตารางด้านล่างเป็นตัวอย่างเมื่อ `Grace_Min = 5` นาที:
