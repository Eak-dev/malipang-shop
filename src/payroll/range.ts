...
export function payrollSnapshotSatang(row:Record<string,unknown>,employee:Employee):number{
  if(row.daily_wage_snapshot_satang!==null&&row.daily_wage_snapshot_satang!==undefined)return Number(row.daily_wage_snapshot_satang);
  if(row.daily_wage_satang!==null&&row.daily_wage_satang!==undefined)return Number(row.daily_wage_satang);
  return employee.dailyWageSatang;
}
export function countsAsPayrollWorkDay(timeIn:string|null,timeOut:string|null):boolean{return timeIn!==null||timeOut!==null;}
function validatePeriod(input:PayrollRangeInput):{fromDate:string;toDate:string;payDate:string}{
  ...
  if(fromDate!==period.weekStart||toDate!==period.weekEnd)throw new Error(`Payroll period must be Thursday-Wednesday: ${period.weekStart} to ${period.weekEnd}`);
