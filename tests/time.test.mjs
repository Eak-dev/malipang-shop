...
test('weekday OCR does not override verified date and time',()=>{const r=validateClock({...base,weekday:'Fri'},'2026-07-19T21:09:00Z',1,.9,30);assert.equal(r.ok,true);assert.equal(r.review,false);assert.equal(r.workDate,'2026-07-20');assert.equal(r.officialTime,'04:08');});
