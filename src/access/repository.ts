...
export async function ensureImportedStaffRole(env:Env,input:{employeeId:string;role?:StaffRole;branchId?:string;status:"ACTIVE"|"INACTIVE"}):Promise<void>{
  ...
  if(existing&&String(existing.status)===status&&String(existing.role)===role&&String(existing.scope)===scope&&(existing.branch_id==null?null:String(existing.branch_id))===branchId)return;
