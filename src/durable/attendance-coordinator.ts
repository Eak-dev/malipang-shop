...
import { approvedOtSatangForDay,preservedWageSnapshot,resolveWageSnapshot,weeklyPayrollStatement } from "../payroll/repository";
import { randomId } from "../shared/ids";
import { minutesOf,payrollPeriodFor } from "../shared/time";
...
export class AttendanceCoordinator{
  ...
  private async commit(input:AttendanceCommitRequest):Promise<AttendanceCommitResult>{
    ...
    const daily=await this.env.DB.prepare(`SELECT time_in,time_out,review_flag,version,daily_wage_snapshot_satang,daily_wage_satang,wage_source_id,other_adjustment_satang FROM attendance_daily WHERE employee_id=? AND work_date=?`).bind(input.employee.employeeId,input.workDate).first<Record<string,unknown>>();
    ...
    const resolved=preservedWageSnapshot(daily)||await resolveWageSnapshot(this.env,input.employee,input.workDate);
    ...
    const nextVersion=current.version+1,payroll=calculatePayroll({employee,timeIn:current.timeIn,timeOut:current.timeOut,review:current.review,otApprovedSatang,otherAdjustmentSatang:Number(daily?.other_adjustment_satang||0)});
    ...
    return{eventId,punchType:punch,workDate:input.workDate,officialTime:input.officialTime,status,lateMinutes:payroll.lateMinutes,confirmedWageSatang:payroll.confirmedWageSatang,pendingWageSatang:payroll.pendingWageSatang,validationCode:input.validationCode,version:nextVersion,weeklyVersion};
  }
}
