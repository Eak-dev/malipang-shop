...
test('sheet jobs are durable before LINE success message',async()=>{
  ...
  const env={
    ...
    EVIDENCE:{async put(){},async get(){return null;}},R2_EVIDENCE_ENABLED:'false',SHEETS_SYNC_ENABLED:'true',RUNTIME_MODE:'production',SHADOW_LINE_OUTPUT:'false',LINE_CHANNEL_ACCESS_TOKEN:'token',EXTERNAL_API_TIMEOUT_MS:'1000',CLOCK_MAX_DATE_DIFF_DAYS:'1',CLOCK_FALLBACK_MIN_CONFIDENCE:'0.9',CLOCK_MAX_LINE_TIME_DIFF_MIN:'30'
  };
  const previousFetch=globalThis.fetch;globalThis.fetch=async()=>{order.push('line-push');return new Response('{}',{status:200});};
  try{
    await handleAttendance(env,{type:'message',timestamp:Date.parse('2026-07-19T21:03:00Z'),source:{type:'user',userId:'U1'},message:{id:'M1',type:'image'},webhookEventId:'W1'},{employeeId:'EMP001',staffName:'Win',lineUserId:'U1',scheduledIn:'04:00',scheduledOut:'16:00',dailyWageSatang:50000,graceMin:10,lateDeductionSatang:0,earlyDeductionSatang:0,canSubmitExpense:false,status:'ACTIVE'},{kind:'CLOCK',hour:4,minute:3,month:7,day:20,weekday:'Mon',confidence:.99,clockFullyVisible:true,needsNewPhoto:false,note:'',provider:'test',raw:null},new Uint8Array([1,2,3]).buffer,'trace_1');
