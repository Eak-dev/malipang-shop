...
export async function handleAttendance(env:Env,event:LineEvent,employee:Employee,reading:VisionResult,original:ArrayBuffer,traceId:string):Promise<boolean>{
  ...
  if(!validated.ok){
    await respondTextToLineEvent(env,event,describeClockValidationFailure(validated.validationCode).message,{traceId,purpose:"ATTENDANCE_REJECTION",identitySuffix:"attendance-rejection"});
    return false;
  }
  ...
  const notification={traceId,purpose:"ATTENDANCE_RESULT" as const,identitySuffix:"attendance-result"};
  if(result.punchType==="COMPLETE"){await respondTextToLineEvent(env,event,buildAttendanceReply(employee,result),notification);return true;}
  const jobs=[
