...
ข้อความ Push และ Flex ที่ใช้แจ้งผลหลังประมวลผลเป็น notification แบบ best effort หาก LINE ตอบ 429 หรือ HTTP error ระบบจะเก็บ `line_push_ms` และ `line_notification_failure` แต่จะไม่เปลี่ยนธุรกรรมที่สำเร็จหรือเหตุการณ์ที่ถูกปฏิเสธอย่างถูกต้องให้เป็น `FAILED` และจะไม่ retry งานธุรกิจทั้ง Event เพียงเพราะส่งข้อความแจ้งเตือนไม่สำเร็จ
