...
export async function handleExpensePostback(env:Env,event:LineEvent,actor:Employee):Promise<void>{
  const q=new URLSearchParams(event.postback?.data||""),action=q.get("a"),id=q.get("id")||"",to=event.source.userId||"",traceId=`postback_${id||"unknown"}`;if(!actor.canSubmitExpense){await pushText(env,to,"ไม่มีสิทธิ์จัดการค่าใช้จ่ายค่ะ",traceId);return;}
  if(action==="expense_confirm"){
    const changed=await env.DB.prepare(`UPDATE expense_events SET status='CONFIRMED',updated_at=? WHERE expense_id=? AND line_user_id=? AND status='WAITING_CONFIRM'`).bind(new Date().toISOString(),id,to).run();if(Number(changed.meta.changes||0)!==1){await pushText(env,to,"รายการนี้ถูกจัดการแล้วหรือไม่ใช่รายการของคุณค่ะ",traceId);return;}
    await enqueueSheetSync(env,{kind:"SHEETS_SYNC",entityType:"EXPENSE",entityKey:id,entityVersion:1,traceId});await pushText(env,to,"ยืนยันบันทึกค่าใช้จ่ายแล้ว ✅",traceId);
