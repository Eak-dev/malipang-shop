...
export async function processInbound(job:InboundJob,env:Env,_ctx:ExecutionContext):Promise<void>{
  ...
  try{
    ...
    if(event.type==="postback"){
      if(env.EXPENSE_ENABLED!=="true"){await pushText(env,to,"ระบบค่าใช้จ่ายยังไม่เปิดใช้งานค่ะ",job.traceId);await completeInboundEvent(env,webhookId,"POSTBACK","IGNORED");return;}
      if(!actor){await pushText(env,to,"ไม่พบสิทธิ์ผู้ใช้งานค่ะ",job.traceId);await completeInboundEvent(env,webhookId,"POSTBACK","REJECTED");return;}
    ...
    if(!actor||(!(actor.status==="ACTIVE"&&env.ATTENDANCE_ENABLED==="true")&&!(actor.canSubmitExpense&&env.EXPENSE_ENABLED==="true"))){await pushText(env,to,"ไม่พบพนักงานหรือสิทธิ์สำหรับรับรูปนี้ค่ะ",job.traceId);await completeInboundEvent(env,webhookId,"UNAUTHORIZED_IMAGE","REJECTED");return;}
    ...
    if(reading.kind==="CLOCK"){
      if(actor.status!=="ACTIVE"||env.ATTENDANCE_ENABLED!=="true"){const code="ATTENDANCE_NOT_ALLOWED";await pushText(env,to,`ยังไม่บันทึกเวลาค่ะ ❌\nสาเหตุ: บัญชีนี้ไม่มีสิทธิ์ลงเวลา หรือระบบลงเวลายังไม่เปิด\nวิธีแก้: กรุณาติดต่อผู้ดูแลร้าน\nรหัส: ${code}`,job.traceId);await completeInboundEvent(env,webhookId,"ATTENDANCE", "REJECTED",code);return;}
      ...
      if(!actor.canSubmitExpense||env.EXPENSE_ENABLED!=="true"){const code="EXPENSE_IMAGE_NOT_ALLOWED";await pushText(env,to,`ยังไม่บันทึกรูปค่าใช้จ่ายค่ะ ❌\nสาเหตุ: AI อ่านพบเอกสารค่าใช้จ่าย แต่บัญชีนี้ไม่มีสิทธิ์หรือระบบยังไม่เปิด\nวิธีแก้: กรุณาติดต่อผู้ดูแลร้าน\nรหัส: ${code}`,job.traceId);await completeInboundEvent(env,webhookId,"EXPENSE_IMAGE","REJECTED",code);return;}
