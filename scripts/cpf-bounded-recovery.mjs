...
export function buildRecoveryPlan(snapshot, runtime, ids = {}) {
  ...
  const statements = [
    statement(
      `INSERT INTO expense_events(expense_id,message_id,line_user_id,description,amount_satang,payment_key,source_wallet,category,transaction_date,status,trace_id,created_at,submitted_by_employee_id,branch_id) SELECT ?,?,?,?,?,?,?,?,?,'WAITING_CONFIRM',?,?,?,? WHERE EXISTS(SELECT 1 FROM expense_documents WHERE document_id=? AND message_id=? AND line_user_id=? AND document_number=? AND final_paid_satang=? AND gross_amount_satang=? AND normalized_json=? AND COALESCE(payment_date,'')=? AND COALESCE(document_date,'')=? AND submitted_by_employee_id IS ? AND branch_id IS ? AND document_type IN ('RECEIPT','TAX_INVOICE','RECEIPT_TAX_INVOICE') AND (LOWER(COALESCE(legal_vendor_name,'')) LIKE '%cpf%' OR LOWER(COALESCE(vendor_name,'')) LIKE '%cpf%') AND status='WAITING_REVIEW' AND expense_id IS NULL) AND (SELECT COUNT(*) FROM expense_document_items WHERE document_id=? AND expense_id IS NULL)=2 AND (SELECT COALESCE(SUM(line_total_satang),0) FROM expense_document_items WHERE document_id=? AND expense_id IS NULL)=? AND NOT EXISTS(SELECT 1 FROM expense_document_cases WHERE document_id=? AND (expense_id IS NOT NULL OR status='CONFIRMED')) AND NOT EXISTS(SELECT 1 FROM expense_document_links WHERE document_id=?) AND NOT EXISTS(SELECT 1 FROM expense_events WHERE message_id=? OR (amount_satang=? AND transaction_date IN (?,?)))`,
      [expenseId, String(documentRow.message_id), String(documentRow.line_user_id), draft.description, draft.amountSatang, draft.paymentKey, draft.sourceWallet, draft.category, draft.transactionDate, traceId, now, actorEmployeeId, branchId, documentId, String(documentRow.message_id), String(documentRow.line_user_id), CPF_DOCUMENT_NUMBER, CPF_FINAL_PAID_SATANG, CPF_GROSS_SATANG, String(documentRow.normalized_json), String(documentRow.payment_date || ""), String(documentRow.document_date || ""), actorEmployeeId, branchId, documentId, documentId, CPF_GROSS_SATANG, documentId, documentId, String(documentRow.message_id), CPF_FINAL_PAID_SATANG, String(documentRow.payment_date || ""), String(documentRow.document_date || "")]
    ),
    statement(
