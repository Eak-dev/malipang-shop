...
test('sheet jobs are durable before retryable Attendance notification is queued',async()=>{
  const order=[];
  ...
  const env={
    ...
    JOB_QUEUE:{async send(body){if(body.kind==='LINE_NOTIFICATION')order.push('line-notification-queue');},async sendBatch(){order.push('sync-job-queue');}},
  ...
  assert.ok(order.indexOf('sync-job-queue')<order.indexOf('line-notification-queue'));
});
