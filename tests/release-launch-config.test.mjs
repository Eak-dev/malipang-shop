...
test("checked-in cutover config is locked to Production with normal LINE output semantics",()=>{
  const config=JSON.parse(fs.readFileSync(new URL("wrangler.jsonc",root),"utf8"));
  assert.equal(config.vars.APP_ENV,"production");
  assert.equal(config.vars.RUNTIME_MODE,"production");
...
test("current operator documentation matches Production cutover and the locked payroll cycle",()=>{
  ...
  assert.match(readme,/\| `APP_ENV` \| `production` \|/);
  assert.match(readme,/\| `RUNTIME_MODE` \| `production` \|/);
