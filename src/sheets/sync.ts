...
export async function syncJob(env:Env,job:SheetsSyncJob,retryAttempt=1):Promise<void>{
  ...
  try{
    ...
    if(job.entityType==="ATTENDANCE_EVENT"){
      ...
      const r=await env.DB.prepare(`SELECT e.*,d.document_id,d.document_type,d.vendor_name,d.document_number,d.order_id FROM expense_events e LEFT JOIN expense_documents d ON d.expense_id=e.expense_id WHERE e.expense_id=? ORDER BY d.created_at ASC LIMIT 1`).bind(job.entityKey).first<Record<string,unknown>>();if(!r)throw new Error(`Expense not found: ${job.entityKey}`);expense=r;sheet=env.SHEET_EXPENSE_RAW;values=[r.expense_id,r.transaction_date,r.description,baht(r.amount_satang),r.payment_key,r.source_wallet,r.category,r.status,r.message_id,r.trace_id,r.submitted_by_employee_id,r.branch_id,r.document_id,r.document_type,r.vendor_name,r.document_number,r.order_id];
