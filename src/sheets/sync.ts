...
export async function syncJob(env:Env,job:SheetsSyncJob,retryAttempt=1):Promise<void>{
  ...
  try{
    let sheet="",values:unknown[]=[],expense:Record<string,unknown>|null=null,expenseDocument:DailyExpenseDocument|null=null,expenseItems:DailyExpenseDocumentItem[]=[];
    ...
    const row=await allocateRow(env,sheet,job.entityKey),end=columnName(values.length),started=Date.now();try{
      ...
      if(expense){
        if(String(expense.status)==="CONFIRMED")await writeConfirmedExpenseWithDocumentItemsToDaily(env,{expenseId:String(expense.expense_id),transactionDate:String(expense.transaction_date),description:String(expense.description),amountBaht:baht(expense.amount_satang),paymentKey:String(expense.payment_key),sourceWallet:String(expense.source_wallet),category:String(expense.category)} satisfies DailyExpenseRecord,expenseDocument,expenseItems);
